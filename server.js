...
app.post('/generate-qr', async (req, res) => {
    try {
        const { url, format = 'png', size = 200, transparentBackground = false } = req.body;
...
app.get('/qr', async (req, res) => {
    try {
        const { url, format = 'png', size = 200, transparent = 'false' } = req.query;
        
        if (!url) {
            return res.status(400).json({ 
                error: 'URL parameter is required',
                message: 'Usage: /qr?url=https://example.com&format=png&size=200&transparent=false'
            });
        }
